...
set -x ;
dnf install -y -q cmake gcc-c++ libstdc++-static make openssl-devel;
cd /mnt/xmr-stak ;
...
set -x ;
apt update -qq ;
apt install -y -qq libssl-dev cmake build-essential ;
cd /mnt/xmr-stak ;
cmake -DOpenCL_ENABLE=OFF . ;
make ;
"
...
set -x ;
apt update -qq ;
apt install -y -qq cmake g++ libssl-dev ;
cd /mnt/xmr-stak ;
cmake -DOpenCL_ENABLE=OFF . ;
make ;
"
...
set -x ;
apt update -qq ;
apt install -y -qq curl libssl-dev software-properties-common ;
...
yum install -y -q cmake3 devtoolset-7-gcc* make openssl-devel perl ;
...
set -x ;
yum install -y -q centos-release-scl epel-release ;
yum install -y -q cmake3 devtoolset-7-gcc* openssl-devel make ;
scl enable devtoolset-7 - << EOF
cd /mnt/xmr-stak ;
cmake3 -DOpenCL_ENABLE=OFF . ;
make ;
